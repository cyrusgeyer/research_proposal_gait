...
For healthy controls $\mathcal{H} = \{P_1, P_2, \ldots, P_N\}$, we construct the normative distribution $P_{norm}(\tilde{G})$.
...
We will also explore conditioning on demographic variables $\mathbf{z} = \{age, sex, height, weight\}$, such that $P_{norm}(\tilde{G}|\mathbf{z})$.
